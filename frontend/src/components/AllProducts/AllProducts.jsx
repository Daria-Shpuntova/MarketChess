...
export default function AllProducts(){
    ...
    useEffect(() => {
        console.log(document.cookie, 'document.cookie1')
        fetch(`http://localhost:8000/api/chessAll`)
            .then(response => {
                if (!response.ok) {
    ...
    const filteredData = selectedColor ? data.filter(prod => prod.color === selectedColor) : data;

    return (
        <section className='containerSecond containerNew'>
            <h2 className='neon'>Каталог</h2>
            <div className='colorFilter'>
                <div>
                    <button onClick={() => setSelectedColor('фиолетовый')}>Фиолетовый</button>
                    <button onClick={() => setSelectedColor('красный')}>Красный</button>
                    <button onClick={() => setSelectedColor('зеленый')}>Зеленый</button>
                    <button onClick={() => setSelectedColor('желтый')}>Желтый</button>
                    <button onClick={() => setSelectedColor('синие')}>Синий</button>
                    <button onClick={() => setSelectedColor('голубой')}>Голубой</button>
                    <button onClick={() => setSelectedColor('микс')}>Микс</button>

                </div>
                <div>
                    <button onClick={() => setSelectedColor('')}>Сбросить фильтр</button>
                </div>
            </div>
            {isModalOpen && <Modal message={modalMessage} onClose={closeModal} />}
            <div className='productsNew'>
            {filteredData.map(prod => (
                    <div key={prod.slug} to={prod.slug} className='product'
                         style={{backgroundImage: `url(${prod.image})`}}>
                        <div className='productCardHead'>
                            <h3 className='neon'>{prod.name}</h3>
                            {prod.discount_percentage ? (
                                <p className='neon'>-{prod.discount_percentage}%</p>) : (<></>)}
                        </div>
                        <div className='productCardFoot'>
                            <div>
                                {prod.discount_percentage ? (
                                    <p><s>{prod.price}</s>
                                        <span
                                            className='neon price'>{prod.price - (prod.price * (prod.discount_percentage / 100))}</span>
                                    </p>
                                ) : (
                                    <p className='neon price'>{prod.price}</p>
                                )}
                            </div>
                            <div className='productSvg'>
                                <svg onClick={() => addToWishlist(prod.id)} className='neon'
                                ...
                                {prod.quantity === 0 ? (
                                        ...
                                        basketItems.some(item => item.id === prod.id) ? (
                                            ...
                                            <>
                                                <svg onClick={() => addToCart(prod.id)} className='neon'
